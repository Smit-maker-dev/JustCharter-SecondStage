...
export default function Fleet() {
  ...
  return (
    <div className="bg-[#F5F5F5] min-h-screen pt-24 pb-24">
      ...
      <div className="max-w-[88rem] mx-auto px-6 mb-32 space-y-32">
        {categories.map((category) => (
          <div key={category.id} className="pt-8 border-t border-gray-200">
            <div className="flex flex-col md:flex-row justify-between items-start mb-12 gap-8">
              <div className="max-w-xl">
                <h2 className="text-4xl font-medium mb-4" style={{ letterSpacing: '-0.03em' }}>{category.title}</h2>
                <p className="text-black/60 text-lg leading-relaxed">{category.description}</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {category.aircraft.map((jet, idx) => (
                <div key={idx} className="bg-white rounded-3xl overflow-hidden group border border-gray-100 flex flex-col hover:shadow-xl transition-shadow duration-300">
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex items-center justify-between mb-6 pt-6 border-t border-gray-100">
                      <span className="text-black/50 font-medium tracking-wide uppercase text-sm">Estimated Price</span>
                      <span className="text-xl font-medium">{jet.estPrice}</span>
                    </div>

                    <div className="flex gap-4">
                      <Link to={`/fleet/${jet.name.toLowerCase().replace(/ /g, '-')}`} className="flex-1 block py-4 text-center text-sm font-medium text-black bg-[#F5F5F5] rounded-full hover:bg-gray-200 transition-colors duration-200">
                        View Details
                      </Link>
                      <button className="flex-1 py-4 text-center text-sm font-medium text-white bg-black rounded-full hover:bg-gray-800 transition-colors duration-200">
                        Request Flight
                      </button>
                    </div>
                  </div>
